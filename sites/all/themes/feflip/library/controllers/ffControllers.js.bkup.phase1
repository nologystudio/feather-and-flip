        ...
        *  
			            ...
			            switch($scope.service){
							case 'expedia':
								if(!_.isUndefined(_data))
									window.location.href = _s + _data.HotelRoomReservationResponse.itineraryId;
								else 
		...
		ffAppControllers.controller('RegistrationCtrl',function($scope,$http){
			...
			$scope.regSubmit = function(_id){
				...
				if(isValid){ //$scope.checkChangedInput()
					$http({
		                method  : 'POST',
		                url     : formSubmit,
		...
		ffAppControllers.controller('HotelCtrl',function($scope){
			...
			$('*[rel="map"]').on('click',function(){
				$scope.showMap = true;
				$('div.map-overlay').show().transition({opacity:1},function(){
					setTimeout(function(){ 
						map.updateSize();
					},200);
				});
			...
			$('div.map-overlay *[rel="close"]').on('click',function(){
				$scope.showMap = false;
				$('div.map-overlay').transition({opacity:0},function(){
					$(this).hide();
				});
			});
