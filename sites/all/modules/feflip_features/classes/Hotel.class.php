...
class Hotel {
  ...
  */
  ...
  public static function GetContentBlocks($node) {
    ...
    if (isset($node->field_contentblocks['und']) && count($node->field_contentblocks['und']) > 0) {
      ...
      foreach ($node->field_contentblocks['und'] as $item) {
        ...
        if (count($contentblocks) > 0 && count($contentblocks[$i]) == 2) {
          $i += 1;
        }

        $contentblocks[$i][] = array(
          'title' => $contentblock->field_caption['und'][0]['value'],
          'features' => $features
        );
      }
    }
  ...
  public static function GetAddressBook($destinationId) {
    ...
    if (!isset($destinationId) || empty(trim($destinationId))) {
      return $adressBook;
    }
  ...
  public static function GetImages($node) {
    ...
    if (count($images) == 0) {
      $images[] = array('url' => "http://placehold.it/1040x650", 'alt' => 'City, Country', 'size' => Helpers::safeGetImageSize("http://placehold.it/1040x650"));
    }
